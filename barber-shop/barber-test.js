...
describe('Barber', function () {
  ...
  it('should be able to have earnings and known haircuts', function () {
    var cut1 = { style: 'mohawk', hairLength: 'short', price: 11.0 };
    var cut2 = { style: 'side part', hairLength: 'medium', price: 12.0 };

    var erin = createBarber('Erin', 23.0, [cut1, cut2]);

    assert.equal(erin.name, 'Erin');
    assert.equal(erin.earnings, 23.0);
    assert.deepEqual(erin.haircuts, [cut1, cut2]);
  });

  it('should default to no earnings and no haircuts if none provided', function () {
    var buzzCut = { style: 'buzz', hairLength: 'short', price: 8.0 };
    var nick = createBarber('Nick', 8.0, [buzzCut]);

    var pam = createBarber('Pam');

    assert.equal(nick.earnings, 8.0);
  ...
  it('should be able to offer a compliment', function () {
    var mohawkCut = { style: 'mohawk', hairLength: 'short', price: 11.0 };
    var buzzCut = { style: 'buzz', hairLength: 'short', price: 8.0 };
  ...
  it('should be able to cut hair', function () {
    ...
    assert.equal(mattCanDoMohawk.haircuts[0].price, 11.0);
  ...
  it('should earn money for hair cuts', function () {
    ...
    var buzzCut = { style: 'buzz', hairLength: 'short', price: 8.0 };
    var sidePartCut = { style: 'side part', hairLength: 'medium', price: 10.0 };

    var erinCanDoBuzz = cutHair(erin, buzzCut);

    assert.equal(erinCanDoBuzz.earnings, 8.0);

    var erinCanDoBuzzAndSidePart = cutHair(erinCanDoBuzz, sidePartCut);

    assert.equal(erinCanDoBuzzAndSidePart.earnings, 18.0);
  });

  it.only('should be able to list style options based on desired length', function () {
    var buzzCut = { style: 'buzz', hairLength: 'short', price: 8.0 };
    var sidePartCut = { style: 'side part', hairLength: 'medium', price: 10.0 };
    var bobCut = { style: 'bob', hairLength: 'short', price: 12.0 };

    var knownCuts = [buzzCut, sidePartCut, bobCut];

    var sandy = createBarber('Sandy', 30.0, knownCuts);
